...
你平时让 AI 干活，活多了就记不清：哪个会话跑完了，哪个卡住在等你点头，定时任务今天到底跑没跑。这个软件就管这件事。装完桌面上多一张小贴纸，每个 AI 会话的状态实时摆在上面，跑完的、正在跑的、等你确认的，一眼看清。长任务跑完了它弹个窗敲一声，你该干嘛干嘛去，不用盯着屏幕等。定时任务没按点跑，它直接报警。

修仙是它的记账方式。你用 AI 干的每件活，它在后台折算成灵气，攒等级、升境界、解锁机缘。用得越多修为越高，从炼气一路修到飞升。攒出来的等级能一键出一张修仙风的等级卡，发群里跟朋友比一比谁修为深。

所有数据都在你自己电脑上算，不联网不上传。装完你该怎么用 AI 还怎么用，它只负责看着、记着、提醒你。
...
目前**实测验证过**的数据源是 Claude Code 和 Codex，两个都用经验直接叠加。

安装时如果检测到你在用其他 AI 工具（Gemini CLI / Qwen Code / Cursor / Cline / Roo Code / Copilot CLI / Windsurf 等），向导会提示「尚未接入」。想让你的工具也算修为：开 Issue 报上工具名，最好附一两个**脱敏后**的本地会话文件样本，拿到样本当天出适配器。每个工具只需要一个几十行的解析器，没有真实样本我们不赌格式，宁缺毋滥。
